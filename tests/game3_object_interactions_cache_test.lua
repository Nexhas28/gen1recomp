  ...
  if key and behavior~=0x83 and behavior~=0x85 then
   ...
   if not covered[behavior] and y+1<layout.height and not shadowed then
    ...
    if SCREEN_ONLY[key] then
     ...
     -- pokefirered/data/scripts/cable_club.inc:566-575 +
     -- pokefirered/src/battle_records.c:83: special ShowBattleRecords + waitstate
     -- parks until the player closes the screen, and releaseall runs only after
     -- the special returns - so control must NOT release while the screen is up.
