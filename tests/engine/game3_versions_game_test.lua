-- T6.3a: the Versions.game(id) facade skeleton.  New file, unwired; this
-- suite pins the fail-closed contract and the FireRed/LeafGreen rows so the
-- T6.2 wiring can alias Versions.game without touching this suite.
--   luajit tests/engine/game3_versions_game_test.lua
...
-- A non-Gen3 process still resolves FireRed, so extractor tooling can require
-- this module without booting a game.
VersionsGame.reset()
GameVersion.set("red")
check(VersionsGame.game(nil) == Versions,
  "a non-Gen3 process fails closed to FireRed's table")

-- -------------------------------------------- the table contract the rows owe
...
-- A broken row does not raise: it warns once and falls back to FireRed.
